...
function normalizeAaronEnlevement(source, sourceId) {
  ...
  const events = [
    ...
    event(
      "IN_TRANSIT",
      source?.scheduleUpdatedAt || source?.transitDate || source?.shippingDate,
      clean(source?.scheduleReason) || "Colis en transit",
      source?.destination
    ),
  ...
  return {
    ...
    company: {
      ...
      name: process.env.TRACKING_COMPANY_NAME || "Transport Fomek",
    },
    shipment: {
