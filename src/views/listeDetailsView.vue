...
import { storage } from "../components/firebaseConfig.js"
...
const trackingLink = computed(() => transportFomekTrackingUrl(client.value?.numero || client.value?.numeroSuivi || client.value?.trackingNumber))
const canSendTrackingLink = computed(() => {
  const statuses = [client.value?.deliveryStatus, ...(client.value?.colis || []).flatMap(item => [item.statutColis, ...(item.details || []).map(detail => detail.statutColis)])]
  return Boolean(trackingLink.value) && (statuses.some(value => {
    const status = String(value || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()
    return status.includes("expedi") || status.includes("transit")
  }) || Boolean(client.value?.transitDate))
})
...
pdf.text("06 95 93 19 92 · transportfomek.fr", 48, 26)
...
async function sendTrackingLinkToShipper() {
  ...
  if (!await confirmToast(`Envoyer le lien TRACKSEND de ${trackingNumber} par SMS à ${recipient} ?`)) return
  ...
  try {
    ...
    const response = await fetch(messagingEndpoint("sendPickupTrackingLink"), {
...
// Génération du PDF sous forme de Blob

    const pdfBlob = pdf.output("blob")

// Création d'un nom de fichier propre

    const safeName = (clientData.expediteur || "bordereau")

      .trim()

      .replace(/[^\w-]+/g, "_")

    const fileName = `factures/${safeName}.pdf`

    console.log("[Firebase Storage] Début upload")

    console.log("[Firebase Storage] Fichier :", fileName)
...
// Référence du fichier dans Firebase Storage
    ...
    const pdfRef = storageRef(storage, fileName)
...
// Upload du PDF
    ...
    console.log(
      ...
      "[Firebase Storage] Upload terminé :",

      snapshot.ref.fullPath

    )

// Récupération de l'URL publique Firebase

    const downloadURL = await getDownloadURL(snapshot.ref)

    console.log(

      "[Firebase Storage] URL générée :",
    ...
    console.error(
      ...
      "[Firebase Storage] Erreur pendant l'upload du PDF :",
    ...
    console.error(
      ...
      "[Firebase Storage] Réponse serveur :",
      ...
      <aside class="min-w-0 space-y-6">
        ...
        <section v-if="isAdminView" class="surface-card p-5">
          ...
          <div class="mt-4 grid gap-3">
            ...
            <a v-if="isSuperAdminView && canSendTrackingLink" :href="trackingLink" target="_blank" rel="noopener" class="block break-all text-sm text-cyan-800 underline">{{ trackingLink }}</a>
